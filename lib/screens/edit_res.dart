...
class EditRes extends StatefulWidget {
  final dynamic data;
  const EditRes({super.key, required this.data});

  @override
  State<EditRes> createState() => _EditResState();
}
...
class _EditResState extends State<EditRes> {
  ...
  getData() async {
    await FirebaseFirestore.instance
      ...
      // _gender.text = value['gender'];
      dropDownValue = value['gender'];
      _dob.text = value['dob'];
      _phone.text = value['phone'];
      _address.text = value['address'];
      _hobbies.text = value['hobbies'];
      _qualification.text = value['qualification'];
      _skills.text = value['skills'];
      _experience.text = value['experience'];
      _certification.text = value['certification'];
      _achievements.text = value['achievements'];
      _projectDetail.text = value['projects'];
    });
  }
              ...
              children: [
                Column(
                  ...
                  children: [
                    ...
                    const Text(
                        "Note: Phone number in non-editable because of due to unique id."),
                    const SizedBox(height: 8.0),
                    const Text(
                      ("Address*"),
                    ),
